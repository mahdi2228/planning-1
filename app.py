...
def planning_state_signature(week_id: int) -> str:
    ...
    orders = q_all(
        "SELECT id,status,remaining,priority,due_date,production_time_h,color_id,updated_at FROM orders ORDER BY id"
    )
...
def apply_agentic_plan(result: Dict[str, Any], user_id: Optional[int]) -> Tuple[bool, str]:
    ...
    with transaction() as conn:
        ...
        for oid, (d, seq, row) in final_free.items():
            ...
            conn.execute("UPDATE orders SET status='PLANIFIE',updated_at=? WHERE id=?", (now_iso(), oid))
